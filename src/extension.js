...
function activate(context) {
  let call = vscode.commands.registerCommand("superencourager.call", () => {
    try {
      ...
      vscode.workspace.onDidChangeConfiguration(function(event) {
        Promise.resolve(
          event.affectsConfiguration("superencourager.type")
        ).then(data => {
          if (data) {
            //true 代表这个属性已经被修改
            timeMeter = undefined;
      ...
      vscode.workspace.onDidChangeConfiguration(function(event) {
        Promise.resolve(
          event.affectsConfiguration("superencourager.timeInterval")
        ).then(data => {
          if (data) {
            //true 代表这个属性已经被修改
            timeMeter = undefined;
  ...
  let setKeyword = vscode.commands.registerCommand(
    "superencourager.setKeyword",
    () => {
  ...
  let switchKeyword = vscode.commands.registerCommand(
    "superencourager.switchKeyword",
    () => {
      let defaultSelect = [
        "邓紫棋",
        "石原里美",
  ...
  let clearImage = vscode.commands.registerCommand(
    "superencourager.clearImage",
    () => {
      ...
      vscode.window.showQuickPick(keywords).then(
        data => {
          ...
          if (data !== ALL_KEYWORD) {
            ...
            // 同时移出动图文件夹
            delImages(
              path.join(context.extensionPath, "/images/" + data + GIF_SUFFIX)
            );
          } else {
  ...
  let showPath = vscode.commands.registerCommand(
    "superencourager.showPath",
    () => {
      vscode.window.showInformationMessage(
        "超级鼓励师本地资源路径：" + getImageRootPath()
      );
    }
  );
